...
every adapter, you'd need to choose them like this:
    ...
    From the 4-jolt rated adapter, the adapters rated 5, 6, or 7 are valid choices. However,
        in order to not skip any adapters, you have to pick the adapter rated 5 jolts (difference
        of 1).
...
The first example above (the one that tarts with 16, 10, 15) supports the following arrangements:
...
INPUT_NUMBERS: List[int] = [int(number) for number in INPUT_FILE.read_text().split("\n")]


def solution_part_1(inputs: List[int]) -> int:
    # Add the 0 rating of the charging outlet, and the (max + 3) rating of device's built-in adapter
    joltage_ratings = sorted(inputs.copy() + [0, max(inputs) + 3])
    one_jolts_diffs, three_jolts_diffs = 0, 0  # initialize counts for each difference
    for last_plug, current_plug in zip(joltage_ratings, joltage_ratings[1:]):
        if current_plug - last_plug == 1:
            one_jolts_diffs += 1
        elif current_plug - last_plug == 2:
            pass
        elif current_plug - last_plug == 3:
            three_jolts_diffs += 1
        else:
            raise ValueError("No continuity can be established")
    return one_jolts_diffs * three_jolts_diffs


def solution_part_2(inputs: List[int]) -> int:
    joltage_ratings = sorted(inputs.copy() + [0, max(inputs) + 3])
    target_output = joltage_ratings[-1]
    number_of_ways: Dict[int, int] = {n: 0 for n in range(target_output + 1)}

    # Initializing trivial cases of rating <= 3
    number_of_ways[0] = 1
    if 1 in joltage_ratings:
        number_of_ways[1] = 1
    if 2 in joltage_ratings and 1 in joltage_ratings:
        number_of_ways[2] = 2
    elif 2 in joltage_ratings:
        number_of_ways[2] = 1

    # Updating values for all other ratings found in 'joltage_ratings'
    for rating in range(3, target_output + 1):
        if rating not in joltage_ratings:
            continue
        number_of_ways[rating] = (
            number_of_ways[rating - 3] + number_of_ways[rating - 2] + number_of_ways[rating - 1]
        )

    return number_of_ways[target_output]


if __name__ == "__main__":
    print(f"Solution for part 1:  {solution_part_1(INPUT_NUMBERS)}")
    print(f"Solution for part 2:  {solution_part_2(INPUT_NUMBERS)}")
