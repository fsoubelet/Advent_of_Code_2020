...
def solution_part_1(inputs: List[int]) -> int:
    for index in range(25, len(inputs)):
        previous_25 = inputs[index - 25 : index]
        possible_combinations = list(combinations(previous_25, 2))
        possible_sums = [sum(combination) for combination in possible_combinations]
        if not any(inputs[index] == sum_result for sum_result in possible_sums):
            return inputs[index]


def solution_part_2(inputs: List[int]) -> int:
    invalid_number = solution_part_1(inputs)

    for start in range(len(inputs)):
        for end in range(start + 1, len(inputs)):
            buffer = inputs[start:end]
